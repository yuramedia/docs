...
2. Di Audio Waveform, seret **penanda merah kiri** untuk mengatur titik masuk (_start_).
3. Seret **penanda merah kanan** untuk mengatur titik keluar (_end_).
...
Tekan `Ctrl+1` atau `Ctrl+2` untuk menyetel waktu _start_ atau _end_ ke frame video yang sedang aktif. Ini berguna untuk sinkronisasi dengan potongan dialog.
...
| Subtitle tidak boleh melewati batas adegan (_scene change_) | —                                |
...
| ----------------- | ------------------ | ------------------------------------------ |
| `{\i1}` … `{\i0}` | Miring (_italic_)  | Suara hati, narasi, lagu                   |
| `{\b1}` … `{\b0}` | Tebal (_bold_)     | Penekanan kuat                             |
...
| `Ctrl+1`  | Set waktu _start_ ke frame saat ini |
| `Ctrl+2`  | Set waktu _end_ ke frame saat ini   |
| `Ctrl+3`  | Play baris aktif saja               |
| `A` / `S` | Geser _start_ mundur/maju 10 ms     |
| `D` / `F` | Geser _end_ mundur/maju 10 ms       |
| `Q` / `W` | Geser _start_ mundur/maju 1 frame   |
| `E` / `R` | Geser _end_ mundur/maju 1 frame     |
...
- Jangan lupa cek _overlap_ antar baris: dua baris yang timing-nya tumpang tindih akan menyebabkan subtitle berkedip.
